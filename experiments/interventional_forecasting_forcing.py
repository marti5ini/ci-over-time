...
class ForcingInterventions:
    def __init__(self, dataset_name: str, normalize: bool, results_csv_path: str, metric: str,
                 ...
                 repetitions: int = 10):
        """
        Initialize the Forcing Interventions Experiment.
    ...
    @staticmethod
    def forcing_forecasting(test_forecast, predicted_forecast, test_effects, predicted_effects):
        test_if = test_effects
        predicted_if = predicted_effects
    ...
    def _run_iteration(self, train_steps: int, repetition: int):
        ...
        true_forcing, predicted_forcing = self.forcing_forecasting(test_forecast, predicted_forecast,
    ...
    def _get_effects(self, simulation: SVARSimulation) -> np.ndarray:
        ...
        simulation, effects = forcing_intervention(simulation, self.intervention_dict,
                                       self.column_names, self.epsilon_scale, self.force,
                                       self.future_steps)
        forcing_effects = clone_effects(self.n_simulation_test, effects)
        return forcing_effects / self.std_array[0] if self.normalize else forcing_effects
...
if __name__ == '__main__':
    ...
    results_csv_path = 'datasets/interventional_forecasting_forcing.csv'
    normalize = True
    collapse = False
    axis = (0, 1)
    ...
    for dataset_name in ['german', 'pendulum']:
        ...
        for metric in ['rmse', 'smape', 'mae']:
            for future_steps in [1, 10]:
                ...
                experiment = ForcingInterventions(
